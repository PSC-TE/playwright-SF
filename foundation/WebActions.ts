...
export class WebActions {
    ...
    async selectFromDropdown(option: string) {
        let isSuccess = false;
        let element = this.page.getByRole('option', { name: option })
        try {
            if (element.isVisible()) {
                await element.waitFor()
                await element.scrollIntoViewIfNeeded()
    ...
    async selectOptionByText(element: Locator, visibleText: string) {
        let isSuccess = false;
        try {
            if (element.isVisible()) {
                await element.waitFor()
      ...
      async navigateToPreviousPage() {
        return await this.page.goBack(); // Navigate back
      }
